...
class SpritefulDragDropFileItem extends SpritefulElement {
  ...
  __uploadFinished(data) {
    ...
    this.fire('upload-complete', { 
      ...
      name, 
      path, 
      url // temp because image will get processed in cloud and changed url
    });
  }
  ...
  __uploadFile(file) {
    if (!this.directory) { 
      throw new TypeError('drag-drop-files must have the directory property set'); 
    }
  ...
  async __removeFileButtonClicked(event) {
    try {
      await this.clicked();
      this.cancelUpload();
